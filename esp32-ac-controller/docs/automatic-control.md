...
`TEMP` 조건은 ON을 한 번 보낸 뒤 측정 온도가 기준보다 0.5°C 이상 낮아져야 다시 대기 상태로 돌아간다. 이 히스테리시스로 기준값 주변에서 같은 신호가 반복되는 것을 막는다. 재부팅 시 이 상태는 초기화되므로 이미 온도가 높은 상태라면 한 번 더 송신할 수 있다.
...
이번 단계에는 자동 OFF 조건을 넣지 않았다. `BOTH`와 `TIME`은 날짜가 바뀌면 다음 송신을 허용한다. 재부팅하면 당일 송신 여부가 초기화되므로 이미 조건이 맞으면 한 번 더 송신할 수 있다. 모든 ON 신호는 전체 상태 패킷이다.
